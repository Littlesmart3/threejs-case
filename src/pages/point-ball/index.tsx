...
const PointBall: React.FC = () => {
  const pointBallRef = useRef<HTMLDivElement>(null);

  // 初始化
  const initPointBall = () => {
    const scene = new THREE.Scene();
    pointBallRef.current?.addEventListener('mousemove', onMouseMove, false);
    ...
    pointBallRef.current?.appendChild(renderer.domElement);
    ...
    for (let i = 0; i < 1600; i++) {
      ...
      direction.x = distance * Math.sin(theta) * Math.cos(phi);
      direction.y = distance * Math.sin(theta) * Math.sin(phi);
      direction.z = distance * Math.cos(theta);
  ...
  return <div ref={pointBallRef} className='point-ball' />;
};

export default PointBall;
